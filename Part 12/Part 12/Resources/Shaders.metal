...
//    output.write(float4(color, 1), gid);
    ...
    //
    float2 cc = 1.1 * float2(0.5 * cos(0.1 * timer) - 0.25 * cos(0.2 * timer),
                             0.5 * sin(0.1 * timer) - 0.25 * sin(0.2 * timer));
...
//    float3 color = float3(dmin.w);
    float3 color = float3(touch.x - touch.y);
    color = mix(color,
    ...
    //
    // Uncomment this part and comment other parts to draw `Lame fractals`
    //
//    uv = uv * 2.0 - 1.0;
//    uv *= 0.5;
//
...
//    uv.x = abs(uv.x);
//
//    float2 n = float2(sin(-(8.0 / 10.0) * 3.1415), cos(-(8.0 / 10.0) * 3.1415));
//    float d = dot(uv - float2(0.5, 0.0), n);
//
//    uv -= n * min(0.0, dot(uv, n)) * 2.0;
//
//    n = float2(sin(-(2.0 / 3.0) * 3.1415), cos(-(2.0 / 3.0) * 3.1415));
//    float scale = 1.0;
//    uv.x += 0.5;
//    for (int i = 0; i < 5; i++) {
//        uv *= 3;
...
//    color += smoothstep(1.0 / height, 0.0, d / scale);
//
//    output.write(float4(color, 1), gid);
}
